...
func (r *ConversationRepository) ListUserConversations(req model.UserConversationsShowRequest, userID int64) ([]model.UserConversation, int, error) {
	...
	if req.IsPublic != nil && *req.IsPublic {
		// If explicitly asking for public, filter by is_public=true
		whereClauses = append(whereClauses, "is_public = TRUE")
	} else {
		// Otherwise, filter by user_id (private conversations)
		whereClauses = append(whereClauses, "user_id = :user_id")
		args["user_id"] = userID
	}

	// Add filters
	filterQuery, filterArgs := postgres.BuildFilterQuery(req.Filters)
	if filterQuery != "" {
	...
	// Add sorting
	sortQuery := postgres.BuildSortQuery(req.Sorts)
	if sortQuery != "" {
	...
	var total int
	nstmt, err := r.db.PrepareNamed(baseCountQuery)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to prepare named statement for count user conversations: %w", err)
	}
	defer nstmt.Close()
	err = nstmt.Get(&total, args)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to count user conversations: %w", err)
	}

	// Add pagination
	baseQuery += fmt.Sprintf(" OFFSET :offset LIMIT :limit")
	args["offset"] = req.PagingOptions.Offset
